...
<div class="content-view-full">
    <div class="class-folder">
		...
		<!--
		function StartUnity( arg )
		{ldelim}

			GetUnity().SendMessage("MainCamera", "setObj", "{concat('http://', ezini('SiteSettings', 'SiteURL', 'site.ini'), '/content/download/',$node.data_map.obj.contentobject_id,'/',$node.data_map.obj.id,'/file/',$node.data_map.obj.content.original_filename)|ezurl('no')}");

			GetUnity().SendMessage("MainCamera", "setTexture","{concat('http://', ezini('SiteSettings', 'SiteURL', 'site.ini'), '/content/download/',$node.data_map.texture.contentobject_id,'/',$node.data_map.texture.id,'/file/',$node.data_map.texture.content.original_filename)|ezurl('no')}");

			GetUnity().SendMessage("MainCamera", "LoadObj","");
		...
		{ldelim}
			alert("hey");
			GetUnity().SendMessage("MainCamera", "setObj", "{concat('http://', ezini('SiteSettings', 'SiteURL', 'site.ini')|explode('/index.php')[0],'/',$node.data_map.obj.content.filepath)}");

			GetUnity().SendMessage("MainCamera", "setTexture","{concat('http://', ezini('SiteSettings', 'SiteURL', 'site.ini')|explode('/index.php')[0],'/',$node.data_map.texture.content[original].full_path)}");
		...
		*}
		...
		<div class="attribute-download">
            {attribute_view_gui attribute=$node.data_map.download}
        </div>
