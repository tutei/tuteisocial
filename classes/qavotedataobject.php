...
class qaVoteDataObject extends eZPersistentObject
{
	 /**
     * Used by {@link qaVoteDataObject::userHasRated()} to cache value.
     * 
	...
	protected $currentUserHasRated = null;
     ...
     /**
     * Construct, use {@link qaVoteDataObject::create()} to create new objects.
     * 
     * @param array $row
     */
    ...
    static function definition()
    {
        static $def = array( 'fields' => array(
                  ...
                  'function_attributes' => array(
                      'vote_object' => 'getVoteObject'
                  ),
                  'increment_key' => 'id',
                  'class_name' => 'qaVoteDataObject',
                  'name' => 'qavote_data' );
        return $def;
    }
     ...
     * Will create a unstored one if none could be fetched ( it will have rating_avrage and vote_count of 0 )
     * 
     * @return qaVoteObject
     */
    function getVoteObject()
    {
        $voteObject = qaVoteObject::fetchByObjectId( $this->attribute('contentobject_id'), $this->attribute('contentobject_attribute_id') );
        if ( !$voteObject instanceof qaVoteObject )
        {
            $voteObject = qaVoteObject::create( array('contentobject_id' => $this->attribute('contentobject_id'),
     ...
     * 1. Session variable 'qaVoteRatedAttributeIdList' for list of attribute_id's
     * 2a. (annonymus user) check against session key
     * 2b. (logged in user) check against user id
     * 
     * @param bool $returnRatedObject Return object if user has rated and [qavote]AllowChangeRating=enabled
     * @return bool|qaVoteDataObject
     */
    function userHasRated()
    {
        if ( $this->currentUserHasRated === null )
        {
            $cond = array( 'contentobject_id' => $this->attribute( 'contentobject_id' ),
                'user_id' => $this->attribute('user_id'), 'contentobject_attribute_id'=>$this->attribute('contentobject_attribute_id'));
            $obj=eZPersistentObject::fetchObject( qaVoteDataObject::definition(), null, $cond );
            if($obj instanceof qaVoteDataObject ) $this->currentUserHasRated=true;
            else $this->currentUserHasRated=false;
        }    	
    ...
    function store( $fieldFilters = null )
    {
        if ( $this->attribute( 'user_id' ) == eZUser::currentUserID() )
        {
            ...
            if ( $http->hasSessionVariable('qaVoteRatedAttributeIdList') )
            {
                $attributeIdList = $http->sessionVariable('qaVoteRatedAttributeIdList') . ',' . $attributeIdList;
            }
            $http->setSessionVariable('qaVoteRatedAttributeIdList', $attributeIdList );
        }
    ...
    /**
     * Create a qaVoteDataObject and store it.
    ...
    /**
     * Create a qaVoteDataObject by definition data (but do not store it, thats up to you!)
