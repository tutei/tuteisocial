<?php
//
// Definition of qaVoteFilter class
//
...
class qaVoteFilter
{
    ...
    function createSqlParts( $params )
    {
         ...
         * {def $rated_articles = fetch( 'content', 'tree', hash(
                                      ...
                                      'extended_attribute_filter', hash( 'id', 'qaVoteFilter', 'params', hash( 'incl_not_rated', false() ) )
                                      ) )}
         * 
         */
        ...
        if ( $leftJoin )
        {
            $sqlFrom = ', ezcontentobject ezco2 LEFT JOIN qavote
                             ON qavote.contentobject_id = ezco2.id ';
        	$sqlWhere = 'ezco2.id = ezcontentobject.id AND';
        }
        else
        {
            $sqlFrom = ', qavote';
            $sqlWhere = 'qavote.contentobject_id = ezcontentobject.id AND';
        }

        return array('tables' => $sqlFrom, 'joins' => $sqlWhere, 'columns' => ', AVG( qavote.rating_average ) as rating,
                             SUM( qavote.vote_count ) as vote_count',
