...
<div class="content-view-full">
    <div class="class-folder">
        ...
        <div class="panda3d-form">
            <form method="post" action={"content/action/"|ezurl}>
                  <input class="button panda3d-new-game" type="submit" name="NewButton" value="Novo Jogo do Panda3d" />
                <input type="hidden" name="ContentNodeID" value="{$node.node_id}" />
                <input type="hidden" name="ContentObjectID" value="{$node.contentobject_id}" />
                <input type="hidden" name="ContentLanguageCode" value="{ezini( 'RegionalSettings', 'ContentObjectLocale', 'site.ini')}" />
                <input type="hidden" name="NodeID" value="{$node.node_id}" />
                <input type="hidden" name="ClassIdentifier" value="panda3d" />
            </form>
        </div>
        {/if}
            ...
            {set $children_count=fetch_alias( 'children_count', hash( 'parent_node_id', $node.node_id,
                                                                      ...
                                                                      'class_filter_array', array('panda3d') ) )}
